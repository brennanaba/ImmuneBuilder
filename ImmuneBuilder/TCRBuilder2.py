    ...
    def __init__(self, numbered_sequences, predictions):
        ...
        with torch.no_grad():
            ...
            self.error_estimates = (
                (self.aligned_traces - self.aligned_traces.mean(0))
                .square()
                .sum(-1)
            )
            self.ranking = [
                x.item() for x in self.error_estimates.mean(-1).argsort()
            ]
    ...
    def save(self, filename=None, check_for_strained_bonds=True):
        ...
        for i in range(len(self.atoms)):
            ...
            if success:
                ...
                self.save_single_unrefined(
                    filename, index=self.ranking.index(i)
                )
                success = refine(
                    filename,
                    filename,
...
class TCRBuilder2:
    def __init__(
        self, model_ids=[1, 2, 3, 4], weights_dir=None, numbering_scheme="imgt"
    ):
        ...
        for id in model_ids:
            ...
            model = StructureModule(
                rel_pos_dim=64, embed_dim=embed_dim[model_file]
            ).to(self.device)
            ...
            try:
                if not are_weights_ready(weights_path):
                    print(
                        f"Downloading weights for {model_file}...", flush=True
                    )
                ...
                print(
                    f"ERROR: {model_file} not downloaded or corrupted.",
                    flush=True,
                )
                raise e
    ...
    def predict(self, sequence_dict):
        numbered_sequences = number_sequences(
            sequence_dict, scheme=self.scheme
        )
        sequence_dict = {
...
def command_line_interface():
    ...
    parser.add_argument(
        "-B",
        "--beta_sequence",
        help="Beta chain amino acid sequence",
        default=None,
    )
    parser.add_argument(
        "-A",
        "--alpha_sequence",
        help="Alpha chain amino acid sequence",
        default=None,
    )
    parser.add_argument(
        "-f",
        "--fasta_file",
        help="Fasta file containing a beta and alpha chain named B and A",
        default=None,
    )
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="Path to where the output model should be saved. Defaults to the same directory as input file.",
        default=None,
    )
    parser.add_argument(
        "--to_directory",
        help="Save all unrefined models and the top ranked refined model to a directory. "
        "If this flag is set the output argument will be assumed to be a directory",
        default=False,
        action="store_true",
    )
    parser.add_argument(
        ...
        help="Don't check for strained bonds. This is a bit faster but will rarely generate unphysical side chains",
        default=False,
        action="store_true",
    )
    parser.add_argument(
    ...
    if args.verbose:
        ...
        print(
            f"Sequences loaded succesfully.\nAlpha and Beta chains are:",
            flush=True,
        )
    ...
    if args.to_directory:
        tcr.save_all(
            args.output, check_for_strained_bonds=check_for_strained_bonds
        )
        if args.verbose:
            print(
                "Refinement finished. Saving all outputs to directory",
                flush=True,
            )
    else:
        tcr.save(
        ...
        if args.verbose:
            outfile = (
                "TCRBuilder2_output.pdb"
                if args.output is None
                else args.output
            )
            print(
                f"Refinement finished. Saving final structure to {outfile}",
                flush=True,
            )
